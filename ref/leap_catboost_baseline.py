...
LEAP Catboost Baseline
Credits to:

Giba Baseline XGBoost
import gc
import time
import cudf
import pickle
...
train = pd.read_parquet(train_files[:4])
gc.collect()

# Validate on last parquet file (625000 samples)
valid = pd.read_parquet(train_files[-1])
gc.collect()
...
scores = []
begin = time.time()
for tnum, target in tqdm(enumerate(targets)):
    if target not in train.columns:
        test[target] = 0.
        score = 1.0
        scores.append(score)
        mean_score = np.mean(scores)
        print(f"{tnum} r2(accum): {mean_score:.4f} / r2({target}): {score:.4f}")
        continue
    M = train[target].mean()
    if (train[target].nunique() != 1) and (train[target].abs().mean() > 1e-3):
        model_path = f"models/catboost_{target}.pkl"
        cat_params = {
            'iterations': 2000, 
            'depth': 8, 
            'task_type' : "GPU",
        ...
        with open(model_path, "wb") as f:
            pickle.dump(model, f)
        score = r2_score(valid[target], y_pred, force_finite=True)
        # If r2 is negative, just use mean(target)
        if score <= 0:
            score = r2_score(valid[target], np.zeros_like(valid[target]), force_finite=True)
            test[target] = 0.
        else:
